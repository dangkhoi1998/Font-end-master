<template>
  <v-responsive class="overflow-y-auto" max-height="315">
    <v-card class="mx-auto animated jackInTheBox fast" style="height: 64px; max-height: 400px" tile v-for="item in items">
...
export default {
  ...
  watch: {
    commentAdd (val) {
      console.log('22', val)
      const indexxx = this.items.findIndex(item => item.note === val.note)
      if (indexxx < 0) {
        this.items.push(val)
      }
    },
